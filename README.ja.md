...
CC Pocket は3ステップで使い始められます。

1. iOS、Android、macOS のいずれかにアプリをインストールする
2. Codex / Claude が入っているマシンで Bridge Server を起動する
3. アプリからターミナルの QR コードをスキャンしてセッションを始める
...
| **Bridge Server** | Codex / Claude を実行するマシンで `npx @ccpocket/bridge@latest` を実行します。 |

Bridge Server を起動すると、ターミナルに QR コードが表示されます。アプリからスキャンすれば接続できます。
...
| **Approval Reviewer** | Default / `Auto Review` | Bridge が対応している場合、Codex の approval reviewer として Auto Review を使えます。 |
