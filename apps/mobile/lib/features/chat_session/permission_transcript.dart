...
PermissionTranscriptStatus? syntheticPermissionOutcome(
  ...
  // Codex's bookkeeping outcomes intentionally omit toolName. Real command
  // output may legitimately be the word "Approved" and must remain visible.
  if (message.toolName != null) return null;
  return switch (message.content.trim()) {
    'Approved' => PermissionTranscriptStatus.approved,
    'Approved (always)' => PermissionTranscriptStatus.approvedForSession,
    'Rejected' => PermissionTranscriptStatus.rejected,
    'Answered' => PermissionTranscriptStatus.answered,
    _ => null,
  };
}
