...
async function checkClaudeAuth(): Promise<AuthCheckResult> {
  // Allow third-party or proxy-backed Claude-compatible runtimes to handle
  // authentication themselves. The SDK/CLI will still surface any real runtime
  // auth errors, but Bridge should not block startup just because Anthropic's
  // official API key env vars are absent.
